...
async def grade_generation_grounded_in_documents_and_question(state: GraphState) -> str:
    """Async quality control with parallel graders"""
    print("---CHECK HALLUCINATIONS & ANSWER QUALITY (PARALLEL)---")
    question = state["question"]
    documents = state["documents"]
    generation = state["generation"]
    retry_count = state.get("generation_retry_count", 0)
    max_retries = state.get("max_generation_retries", 1)  # Reduced from 3 to 1

    # Run both graders in parallel using asyncio.gather
    try:
        print("---RUNNING PARALLEL QUALITY CHECKS---")
        
        # Create tasks for parallel execution
        hallucination_task = hallucination_grader.ainvoke(
            {"documents": documents, "generation": generation}
        )
        answer_task = answer_grader.ainvoke(
            {"question": question, "generation": generation}
        )
        
        # Wait for both tasks to complete in parallel
        hallucination_result, answer_result = await asyncio.gather(
            hallucination_task, answer_task
        )
        
        hallucination_grade = hallucination_result.binary_score
        answer_grade = answer_result.binary_score
        
    except Exception as e:
        print(f"---ERROR IN PARALLEL GRADING: {e}---")
